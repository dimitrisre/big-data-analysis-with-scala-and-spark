...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType ={
    val idName = columnNames.head
    val idCol = StructField(idName, StringType, nullable = false)
    val restColumns = columnNames.tail.map(
      name => StructField(name, DoubleType, nullable = false)
    )
    StructType(idCol :: restColumns)
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val primary = columnNames.filter{
      c => primaryNeeds.exists(p => c.startsWith(p))
    }.map(col)

    val working = columnNames.filter{
      c => workingActivities.exists(w => c.startsWith(w))
    }.map(col)

    val other = columnNames.filter{
      c => {
        otherActivities.exists(o => c.startsWith(o)) &&
          !primaryNeeds.exists(p => c.startsWith(p)) &&
          !workingActivities.exists(w => c.startsWith(w))
      }
    }.map(col)
  ...
  def timeUsageSummary(
    ...
    val primaryNeedsProjection: Column = (
      primaryNeedsColumns.reduceLeft(_+_) / 60
      ).as("primaryNeeds")

    val workProjection: Column = (
      workColumns.reduceLeft(_+_)/60
      ).as("work")

    val otherProjection: Column = (
      otherColumns.reduceLeft(_+_)/60
      ).as("other")
